...
import { textToSpeech, translateText } from "../services/translationApi";
...
export const createTranscriptFxn = async (
  req: express.Request,
  ...
  try {
    ...
    const { firstLanguage, secondLanguage } = req.body as {
...
export const getTranscriptByIdFxn = async (
  req: express.Request,
  res: express.Response
) => {
  try {
...
export const translateTextFxn = async (
  req: express.Request,
  res: express.Response
) => {
  try {
    ...
    const { originalText, originalLanguage, translatedLanguage } = req.body as {
      originalText: string;
      originalLanguage: LanguageCodesType;
      translatedLanguage: LanguageCodesType;
    };

    const translation = await translateText(
      originalText,
      languageCodes[originalLanguage],
      languageCodes[translatedLanguage]
    );
    ...
    if (
      languageCodes[translatedLanguage] === "ee" ||
      languageCodes[translatedLanguage] === "tw"
    ) {
        tts = await textToSpeech(
            translation,
            languageCodes[translatedLanguage]
          );
    }
    ...
    const storedTranslation = await storeTranslation({
      ...
      audioUrl: `${req.protocol}://${req.get("host")}/audio/${tts}`,
    });
